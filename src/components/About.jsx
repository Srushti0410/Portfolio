...
export default function About() {
  ...
  const stats = [
    { label: 'HACKATHONS', val: '04+' },
    { label: 'PROJECTS SHIPPED', val: '10+' },
  ];

  return (
    <section id="about" className="py-24 sm:py-32 w-full bg-[#050505] relative border-t border-white/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          >
            ...
            <div className="w-full pt-6 sm:pt-8 border-t border-white/10 grid grid-cols-2 gap-6 sm:gap-8">
              {stats.map((stat, idx) => (
                <div key={stat.label} className={`flex flex-col ${idx === 1 ? 'pl-6 sm:pl-8 border-l border-white/10' : ''}`}>
                  <span className="font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-white tracking-tight">
                    {stat.val}
                  </span>
                  <span className="text-[10px] sm:text-xs font-mono tracking-widest text-gray-400 uppercase mt-1">
                    {stat.label}
                  </span>
                </div>
              ))}
          ...
          <motion.div
            ...
            className="w-full"
          >
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 sm:p-10 relative overflow-hidden shadow-2xl">
              ...
              <div className="flex flex-wrap gap-2.5 sm:gap-3">
                {skills.map((skill, index) => (
                  <motion.div
                    ...
                    className="px-3.5 sm:px-5 py-2.5 sm:py-3 rounded-full bg-[#0c0c0e] border border-white/10 text-gray-200 text-xs sm:text-sm font-medium tracking-wide cursor-default transition-all duration-300 hover:border-[#ff0055] hover:text-[#ff0055] hover:shadow-[0_0_15px_rgba(255,0,85,0.4)] flex items-center gap-2 group"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-gray-500 group-hover:bg-[#ff0055] transition-colors" />
                    <span>{skill}</span>
                  </motion.div>
                ))}
              </div>
